...
#  # make the network
  ...
  docker run --network doge-peers --name doge_node_observer --rm -d -p 28332:28332 -v $DIRECTORY/volumes/doge/node-3:/root/.dogecoin doge-regtest -debug=1 -zmqpubhashblock=tcp://0.0.0.0:28332 -zmqpubhashtx=tcp://0.0.0.0:28332 -zmqpubrawblock=tcp://0.0.0.0:28332 -zmqpubrawtx=tcp://0.0.0.0:28332 -zmqpubsequence=tcp://0.0.0.0:28332
  ...
  echo "disconnecting doge_node_observer from doge-peers network"
  docker network disconnect doge-peers doge_node_observer
  echo "mining 1 blocks on doge_node_2"
  docker exec doge_node_2 dogecoin-cli --rpcuser=user --rpcpassword=hackme -regtest generate 1
  echo "pausing to allow user to get peer info. execute the following to getchaintips. Press enter to continue"
  echo "docker exec doge_node_observer dogecoin-cli --rpcuser=user --rpcpassword=hackme -regtest getchaintips"
  echo "docker exec doge_node_2 dogecoin-cli --rpcuser=user --rpcpassword=hackme -regtest getchaintips"
    read -r
  ...
  echo "mining 1 blocks on doge_node_1 to create a longest chain, and valid fork for doge_node_1"
  docker exec doge_node_2 dogecoin-cli --rpcuser=user --rpcpassword=hackme -regtest generate 1

  echo "pausing to allow user to get peer info. execute the following to getchaintips. Press enter to continue"
  echo "docker exec doge_node_2 dogecoin-cli --rpcuser=user --rpcpassword=hackme -regtest getchaintips"
  echo "docker exec doge_node_1 dogecoin-cli --rpcuser=user --rpcpassword=hackme -regtest getchaintips"
  read -r

  echo "re-connecting doge_node_observer to doge-peers network"
  docker network connect doge-peers doge_node_observer
